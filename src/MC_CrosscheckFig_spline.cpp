...
{ 
 ...
 //////////////////////
   ...
   canvas_v2_vs_pT.SetTopMargin(0.05);
	...
	canvas_v2_vs_pT.DrawFrame(v2vspt_ptmin,v2vspt_v2min,v2vspt_ptmax,v2vspt_v2max);
   ...
   cpar_GENE->GetXaxis()->SetLimits(v2vspt_ptmin,v2vspt_ptmax);
	...
	TF1 *cparv2_fit = new TF1 ("cparv2_fit", "[1]*x+[2]*x*x+[3]*x*x*x",0,3);
	TF1 *pionv2_fit = new TF1 ("pionv2_fit", "[1]*x+[2]*x*x+[3]*x*x*x",0,0.9);
	TF1 *kaonv2_fit = new TF1 ("kaonv2_fit", "[1]*x+[2]*x*x+[3]*x*x*x",0,0.9);
	TF1 *protv2_fit = new TF1 ("protv2_fit", "[1]*x+[2]*x*x+[3]*x*x*x",0,1.5);

	TSpline3 *cpar_sp = new TSpline3("cpar_sp",cpar_GENE);
	TSpline3 *pion_sp = new TSpline3("pion_sp",pion_GENE);
	TSpline3 *kaon_sp = new TSpline3("kaon_sp",kaon_GENE);
	TSpline3 *prot_sp = new TSpline3("prot_sp",prot_GENE);

	cpar_sp->SetLineColor(kBlack);
	pion_sp->SetLineColor(kRed);
	kaon_sp->SetLineColor(kGreen);
	prot_sp->SetLineColor(kBlue);

	cpar_sp->SetLineStyle(9);
	pion_sp->SetLineStyle(9);
	kaon_sp->SetLineStyle(9);
	prot_sp->SetLineStyle(9);
	...
	{
		cpar_GENE_ypts[ptBin]  = cpar_GENE_ypts[ptBin]  / cpar_sp->Eval( cpar_RECO_xpts[ptBin] );
		cpar_RECO_ypts[ptBin]  = cpar_RECO_ypts[ptBin]  / cpar_sp->Eval( cpar_RECO_xpts[ptBin] );
		cpar_GENE_Eypts[ptBin] = cpar_GENE_Eypts[ptBin] / cpar_sp->Eval( cpar_RECO_xpts[ptBin] );
		cpar_RECO_Eypts[ptBin] = cpar_RECO_Eypts[ptBin] / cpar_sp->Eval( cpar_RECO_xpts[ptBin] );
	}

	for(int ptBin = 0; ptBin < nPtBins[1]; ptBin++)
	{
		pion_GENE_ypts[ptBin]  = pion_GENE_ypts[ptBin]  / pion_sp->Eval( pion_RECO_xpts[ptBin] );
		pion_RECO_ypts[ptBin]  = pion_RECO_ypts[ptBin]  / pion_sp->Eval( pion_RECO_xpts[ptBin] );
		pion_GENE_Eypts[ptBin] = pion_GENE_Eypts[ptBin] / pion_sp->Eval( pion_RECO_xpts[ptBin] );
		pion_RECO_Eypts[ptBin] = pion_RECO_Eypts[ptBin] / pion_sp->Eval( pion_RECO_xpts[ptBin] );
	}

	for(int ptBin = 0; ptBin < nPtBins[2]; ptBin++)
	{
		kaon_GENE_ypts[ptBin]  = kaon_GENE_ypts[ptBin]  / kaon_sp->Eval( kaon_RECO_xpts[ptBin] );
		kaon_RECO_ypts[ptBin]  = kaon_RECO_ypts[ptBin]  / kaon_sp->Eval( kaon_RECO_xpts[ptBin] );
		kaon_GENE_Eypts[ptBin] = kaon_GENE_Eypts[ptBin] / kaon_sp->Eval( kaon_RECO_xpts[ptBin] );
		kaon_RECO_Eypts[ptBin] = kaon_RECO_Eypts[ptBin] / kaon_sp->Eval( kaon_RECO_xpts[ptBin] );
	}

	for(int ptBin = 0; ptBin < nPtBins[3]; ptBin++)
	{
		prot_GENE_ypts[ptBin]  = prot_GENE_ypts[ptBin]  / prot_sp->Eval( prot_RECO_xpts[ptBin] );
		prot_RECO_ypts[ptBin]  = prot_RECO_ypts[ptBin]  / prot_sp->Eval( prot_RECO_xpts[ptBin] );
		prot_GENE_Eypts[ptBin] = prot_GENE_Eypts[ptBin] / prot_sp->Eval( prot_RECO_xpts[ptBin] );
		prot_RECO_Eypts[ptBin] = prot_RECO_Eypts[ptBin] / prot_sp->Eval( prot_RECO_xpts[ptBin] );
	}
   ...
   cpar_RECO_norm.GetXaxis()->SetLimits(rv2vspt_ptmin,rv2vspt_ptmax);
	...
	TLine *line_11  = new TLine(rv2vspt_ptmin,1.1,rv2vspt_ptmax,1.1); 
	TLine *line_10  = new TLine(rv2vspt_ptmin,1.0,rv2vspt_ptmax,1.0); 
	TLine *line_09  = new TLine(rv2vspt_ptmin,0.9,rv2vspt_ptmax,0.9); 

	line_11->SetLineStyle(7);
	line_10->SetLineStyle(7);
	line_09->SetLineStyle(7);
